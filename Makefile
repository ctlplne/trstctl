...
# After the core-family moves, the remaining ee/ measured 70.7% on 2026-09-23
# in the complete race/coverage ee-test gate. Raise the floor from 65 to 70
# (R-09 shared edition consequences): 0.7 points of measured margin instead of
# retaining a floor calibrated against the former, larger ee/ tree. Never lower it.
EE_COVERAGE_MIN ?= 70
...
soak: ## Run the endurance/soak gate self-test: fail on an induced leak, pass on a healthy series (PERF-004)
	@set -e; out="$${SOAK_OUT:-$${TMPDIR:-/tmp}/trstctl-soak.json}"; \
...
claim-traceability-check: ## Verify ee/docs/claim-traceability.md regenerates byte-identical from core and ee/ claim citations, with zero integrity findings
...
security-review: editions-gate ## Security-focused local review for signer, remediation, connectors, XREC, and VDEC delivery paths
	...
	@$(GO) test -tags integration ./internal/reconcile/intwire/... ./internal/reconcile/conformance/... ./internal/decommission/intwire/... ./internal/decommission/conformance/... -count=1 -timeout=12m
...
sca: ## Software-composition analysis across all dependency surfaces (Go + npm + embedded-postgres)
	...
	@echo ">> npm dependency tree SCA (web + TypeScript SDK generator + Pulumi IaC)"; bash scripts/ci/npm-audit-dependency-surfaces.sh
...
web-contract: ## Regenerate FE OpenAPI and capability contracts; commit the reviewed diff
	...
	cd web && npm run gen:feature-contracts

.PHONY: feature-parity-check
feature-parity-check: ## Fail on invalid capability stages, stale generated contracts, backend-without-UI, or ghost UI
	$(GO) test $(GO_TEST_EXACT_FLAG) ./internal/featureparity ./tools/featureparityreport
	cd web && npm run gen:feature-contracts -- --check
	cd web && npm run test -- --run web/src/__tests__/feature_contract.test.ts web/src/__tests__/feature_contract_parity.test.ts
