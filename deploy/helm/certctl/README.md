# certctl control-plane Helm chart

Deploys the certctl **control plane** (API + web UI) to Kubernetes with the
...
- **`certctl`** — the control plane: serves the API/UI on `:8443` (HTTPS by
