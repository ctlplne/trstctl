...
// ee/agentid* exported constructor must have at least one NON-TEST caller. The DEFERRED
// allow-list (ee/agentid/verify -- the external RP SDK) is excepted, mirroring the PCAS
// gate's DEFERRED tier. This test MUST pass on the current tree (the AGID-INT-CALL
// wiring landed: the cmd/trstctl attach -> ee/agentid/api -> ee/agentid/orchestrator
...
func TestProdCaller_EveryConstructorHasNonTestCaller(t *testing.T) {
	...
	if len(offenders) > 0 {
		...
		t.Fatalf("AGID-INT-CALL FLOOR FAILED: %d REQUIRED ee/agentid constructor(s) have ONLY test-only callers (a mechanism built and unit-tested but never wired into a running binary). Wire each on its own mechanism card (AGID-01…11), do NOT add product code in intgate:\n  - %s",
...
// packages. Adding a NEW exported ee/agentid constructor without classifying it here
...
func TestDeferredAllowList_IsMinimal(t *testing.T) {
	wantPkgs := map[string]bool{"ee/agentid/verify": true}
	...
	wantSyms := map[string]bool{"ee/agentid/agentstack\x00New": true}
