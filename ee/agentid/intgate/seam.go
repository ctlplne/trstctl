...
// seamFileHasCoreExclusionTag reports whether a sanctioned seam file carries the
// //go:build !trstctl_core constraint (so it is linked in the default/EE build and
// dropped in the core build). The seam assertion requires this: the seam must be the
// EE-only attach, not linked into core.
