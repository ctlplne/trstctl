...
// (broker.New rejects a nil Issuer). On the chain-bound path this issuer is NEVER invoked
// to mint: the brokerstore precondition fails closed BEFORE broker.Issue runs (the
// AGID-04 in-signer gate is provisioned only in AGID-INT-WIRE), so no ephemeral.Issue call
// is reached. It exists so the broker type-checks and the chain-bound CALL PATH is
// reachable; a real, CA-backed ephemeral issuer is provisioned in AGID-INT-WIRE.
//
...
// path ever reached it before INT-WIRE, it would refuse — never mint an unattested badge.
...
// unreachable on the chain-bound path (the precondition refuses first), but if reached it
// refuses rather than minting.
var errIssuerNotProvisioned = errors.New("agentid orchestrator: ephemeral issuer not provisioned (AGID-INT-WIRE); chain-bound path refuses in the precondition before issuance")
...
// non-nil Issuer invariant while the real CA-backed issuer awaits AGID-INT-WIRE. A build
// error is impossible for these fixed inputs, so a nil issuer on error is acceptable —
// broker.New would then reject it fail-closed.
