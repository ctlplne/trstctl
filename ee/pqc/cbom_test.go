...
func TestCBOMTargetForNamesFIPSReplacements(t *testing.T) {
	cases := []struct {
		...
		{"hybrid stays transitional", cbom.Finding{Algorithm: string(HybridMLDSA44ECDSAP256Algorithm)}, "ML-DSA-65", "FIPS 204", cbom.GenerationMigrationRequired, true},
...
func TestCBOMClassifyKeyRecognizesPostQuantumFamilies(t *testing.T) {
	for _, alg := range []string{
		string(MLDSA44), string(MLDSA65), string(MLDSA87),
		string(MLKEM512), string(MLKEM768), string(MLKEM1024),
		string(SLHDSA128s), string(SLHDSA256s),
	...
	hybrid, ok := CBOMClassifyKey(string(HybridEd25519Dilithium3), 0)
...
func TestProgressCountsLicensedFutureReady(t *testing.T) {
	...
	findings := []cbom.Finding{
		...
		cbom.Finding{Kind: cbom.AssetCertKey, Location: "b", Algorithm: string(MLDSA65)}.Classified(policy),
		cbom.Finding{Kind: cbom.AssetTLSEndpoint, Location: "c", Protocol: "TLSv1.2"}.Classified(policy),
		cbom.Finding{Kind: cbom.AssetCertKey, Location: "d", Algorithm: string(HybridMLDSA44ECDSAP256Algorithm)}.Classified(policy),
	}
	...
	if rsaTarget.Algorithm != string(MLDSA65) || rsaTarget.Standard != "FIPS 204" {
