...
// check: an RTA call graph over the whole program proves each wired ee/succession
...
func TestProdCaller_ReachableFromBinaryMain(t *testing.T) {
	...
	t.Fatalf("PCAS-INT-CALL STRONG CHECK FAILED: %d ee/succession constructor(s) are not RTA-reachable from an in-scope binary main built without -tags trstctl_core:\n  - %s",
