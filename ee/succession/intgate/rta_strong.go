...
// inScopeBinaries are the cmd/* mains the RTA analysis seeds from. All three are
// compiled WITHOUT -tags trstctl_core so the //go:build !trstctl_core EE seams link:
// cmd/trstctl attaches the PCAS API and outbox worker, cmd/trstctl-signer the in-signer
// minter, and cmd/trstctl-agent the co-signer service.
...
func rtaReachability() (unreachable []Constructor, loadedPkgCount int, err error) {
	...
	for _, p := range prog.AllPackages() {
		if p == nil || p.Pkg == nil || !isInScopeMain(p.Pkg.Path()) {
			continue
		}
