...
func normalizeSCIM(cfg *api.SCIMConfig) map[string]scimToken {
...
func (a *scimHandler) scimTenant(r *http.Request) (scimToken, string, bool) {
...
func (a *scimHandler) scimServiceProviderConfig(w http.ResponseWriter, r *http.Request) {
...
func (a *scimHandler) scimCreateUser(w http.ResponseWriter, r *http.Request) {
...
func (a *scimHandler) scimListUsers(w http.ResponseWriter, r *http.Request) {
...
func (a *scimHandler) scimGetUser(w http.ResponseWriter, r *http.Request) {
	tok, ok := a.scimRead(w, r)
	if !ok {
		return
	}
...
func (a *scimHandler) scimPutUser(w http.ResponseWriter, r *http.Request) {
	tok, raw, ok := a.prepareSCIMMutation(w, r)
	if !ok {
		return
	}
...
func (a *scimHandler) scimPatchUser(w http.ResponseWriter, r *http.Request) {
	tok, raw, ok := a.prepareSCIMMutation(w, r)
	if !ok {
		return
	}
...
func (a *scimHandler) scimDeleteUser(w http.ResponseWriter, r *http.Request) {
	tok, raw, ok := a.prepareSCIMMutation(w, r)
	if !ok {
		return
	}
...
func (a *scimHandler) applySCIMUser(ctx context.Context, tok scimToken, subject string, in scim.User) (store.TenantMember, error) {
...
func (a *scimHandler) applySCIMUserLocked(ctx context.Context, tok scimToken, subject string, in scim.User) (store.TenantMember, error) {
...
func (a *scimHandler) scimCreateGroup(w http.ResponseWriter, r *http.Request) {
	tok, raw, ok := a.prepareSCIMMutation(w, r)
	if !ok {
		return
	}
...
func (a *scimHandler) scimListGroups(w http.ResponseWriter, r *http.Request) {
	tok, ok := a.scimRead(w, r)
	if !ok {
		return
	}
...
func (a *scimHandler) scimGetGroup(w http.ResponseWriter, r *http.Request) {
	tok, ok := a.scimRead(w, r)
	if !ok {
		return
	}
...
func (a *scimHandler) scimPatchGroup(w http.ResponseWriter, r *http.Request) {
	tok, raw, ok := a.prepareSCIMMutation(w, r)
	if !ok {
		return
	}
...
func (a *scimHandler) scimDeleteGroup(w http.ResponseWriter, r *http.Request) {
	tok, raw, ok := a.prepareSCIMMutation(w, r)
	if !ok {
		return
	}
...
func (a *scimHandler) addRoleToMember(ctx context.Context, tok scimToken, subject, roleName string) error {
	tenantID := tok.TenantID
...
func (a *scimHandler) removeRoleFromMember(ctx context.Context, tok scimToken, subject, roleName string) error {
	tenantID := tok.TenantID
	member, err := a.store.GetTenantMember(ctx, tenantID, subject)
	if err != nil {
...
func (a *scimHandler) groupToSCIM(ctx context.Context, tok scimToken, roleName, base string) (scim.Group, error) {
	tenantID := tok.TenantID
	members, err := a.store.ListTenantMembersByRole(ctx, tenantID, roleName)
	if err != nil {
...
func (a *scimHandler) scimRead(w http.ResponseWriter, r *http.Request) (scimToken, bool) {
	tok, hash, ok := a.scimTenant(r)
	if !a.allowSCIMSpecialRouteRequest(w, r, tok, hash, ok) {
		return scimToken{}, false
	}
...
func (a *scimHandler) allowSCIMSpecialRouteRequest(w http.ResponseWriter, r *http.Request, tok scimToken, tokenHash string, matched bool) bool {
	tokenKey, tenantID := "", ""
	if tokenHash != "" {
		tokenKey = "scim:" + tokenHash
	}
	if matched {
		tenantID = tok.TenantID
	}
	if a.allowRequest == nil {
		writeSCIMError(w, http.StatusServiceUnavailable, "", "SCIM route admission is not configured")
		return false
	}
	return a.allowRequest(w, r, tenantID, tokenKey)
}

func (a *scimHandler) prepareSCIMMutation(w http.ResponseWriter, r *http.Request) (scimToken, []byte, bool) {
	tok, ok := a.scimRead(w, r)
	if !ok {
...
func (a *scimHandler) scimMutate(w http.ResponseWriter, r *http.Request, tok scimToken, idempotencyKey, previousBucketKey string, requestBody []byte, fn func(ctx context.Context, tenantID string) (int, any, error)) {
...
func (a *scimHandler) writeSCIMCached(w http.ResponseWriter, cacheRaw []byte, binding string) {
...
// NewSCIMHandler mounts the tenant protocol on the core-owned dependencies.
func NewSCIMHandler(cfg api.SCIMConfig, d api.SCIMHandlerDeps) http.Handler {
	tokens := normalizeSCIM(&cfg)
	if len(tokens) == 0 {
		return nil
	}
	h := &scimHandler{store: d.Store, orch: d.Orchestrator, idem: d.Idempotency, roles: d.Roles, scimTokens: tokens, allowRequest: d.AllowRequest}
	mux := http.NewServeMux()
	mux.HandleFunc("GET /scim/v2/ServiceProviderConfig", h.scimServiceProviderConfig)
	mux.HandleFunc("POST /scim/v2/Users", h.scimCreateUser)
	mux.HandleFunc("GET /scim/v2/Users", h.scimListUsers)
	mux.HandleFunc("GET /scim/v2/Users/{id}", h.scimGetUser)
	mux.HandleFunc("PUT /scim/v2/Users/{id}", h.scimPutUser)
	mux.HandleFunc("PATCH /scim/v2/Users/{id}", h.scimPatchUser)
	mux.HandleFunc("DELETE /scim/v2/Users/{id}", h.scimDeleteUser)
	mux.HandleFunc("POST /scim/v2/Groups", h.scimCreateGroup)
	mux.HandleFunc("GET /scim/v2/Groups", h.scimListGroups)
	mux.HandleFunc("GET /scim/v2/Groups/{id}", h.scimGetGroup)
	mux.HandleFunc("PATCH /scim/v2/Groups/{id}", h.scimPatchGroup)
	mux.HandleFunc("DELETE /scim/v2/Groups/{id}", h.scimDeleteGroup)
	return mux
}
...
// Preserve the persisted idempotency wire envelope across the package move.
