...
func TestProdCaller_EveryConstructorHasNonTestCaller(t *testing.T) {
	...
	if len(offenders) > 0 {
		...
		t.Fatalf("VDEC-INT-CALL FLOOR FAILED: %d ee/decommission constructor(s) have ONLY test-only callers. Wire each mechanism on its owning VDEC card; do not add product logic in intgate:\n  - %s",
