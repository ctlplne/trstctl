...
| `licenseboundary` | **PACKAGING-007** | Core Go files must carry `SPDX-License-Identifier: BUSL-1.1`, `clients/` Go files must carry `SPDX-License-Identifier: MPL-2.0`, `ee/` Go files must carry `SPDX-License-Identifier: LicenseRef-trstctl-EE`, and core may not import `ee/` outside the tagged attach seams. |
