...
| `netexec` | **SEC-005** | New outbound HTTP and process-exec surfaces must use SSRF-safe clients or reviewed validated-argv paths: ambient `http.DefaultClient`, ambient `http.Client` composite-literal construction, the package-level `http.Get`/`Head`/`Post`/`PostForm` helpers (which use `DefaultClient` implicitly), new `exec.Command` call sites, and direct shell interpreter execution all fail closed. Only `internal/netsec` and `internal/egress` — the packages that implement the sanctioned client — may construct one freely; the callers that predate the rule sit in a burn-down allowlist that a test forces to shrink, never grow. |
| `licenseboundary` | **PACKAGING-007** | Core Go files must carry `SPDX-License-Identifier: BUSL-1.1`, `clients/` Go files must carry `SPDX-License-Identifier: MPL-2.0`, `ee/` Go files must carry `SPDX-License-Identifier: LicenseRef-trstctl-EE`, core may not import `ee/` outside the tagged attach seam, and PQC-related code is rejected from core. |
...
| `upsertarbiter` | **OPP-C01** | An `INSERT ... ON CONFLICT` upsert into a table that carries a second unique index must serialize on `pg_advisory_xact_lock` or handle `unique_violation` (23505): Postgres's speculative insertion can raise 23505 on the non-arbiter index while two identical inserts race (DP2-043/DP2-046). Scoped to `internal/store`; pre-existing sites sit in a reviewed, pinned baseline so a new site fails closed. |
