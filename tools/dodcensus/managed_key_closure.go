...
func inspectManagedKeyRuntimeClosure(repo string, substrate Substrate) error {
	...
	for _, required := range []string{
		...
		`dodManagedKeyPostgresImage                 = "postgres:16.15-alpine@sha256:cf78e76683b9ca8c5733cbbdce6c9262b45b6767934dd0a95e671f9a0fc20685"`,
		...
		`"docker", "network", "create", "--driver", "bridge", "--internal", route.network`,
		`"docker", "network", "connect", route.network, route.runnerContainer`,
		`if route.publish != ""`,
		`if len(bindings) != 0`,
		`row.Driver != "bridge" || !row.Internal || row.Ingress || len(row.Containers) != 2`,
