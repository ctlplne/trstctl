...
func TestEndpointSubjectChoiceSurvivesReplayAndRefusesRepurposing(t *testing.T) {
	ctx := t.Context()
	st := newStore(t)
	log := openLog(t)
	...
	if err := json.Unmarshal([]byte(`{"subject_key_algorithm":"ML-DSA-65"}`), &issuer); err != nil {
		t.Fatal(err)
	}
	...
	assertBinding := func() {
		...
		for key, want := range map[string]string{
			"discovery_source": "local-scan", "issuing_authority_source": "external", "subject_key_algorithm": "ML-DSA-65",
	...
	assertBinding()
	otherAlgorithm := issuer
	if err := json.Unmarshal([]byte(`{"subject_key_algorithm":"ECDSA-P256"}`), &otherAlgorithm); err != nil {
		t.Fatal(err)
	}
	assertRefused(otherAlgorithm, tenantA)
	otherIssuer := issuer
