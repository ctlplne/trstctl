...
// Connector writes RabbitMQ TLS certificate and key files.
...
// New returns a RabbitMQ connector for the configured server cert and key.
func New(certPath, keyPath string) *Connector {
	return &Connector{certPath: certPath, keyPath: keyPath, reload: []string{"rabbitmqctl", "rotate_certs"}}
}

// Name identifies the connector.
func (c *Connector) Name() string { return "rabbitmq" }

// Capabilities grants read/write on configured TLS directories and process exec
// for the direct rotate command.
func (c *Connector) Capabilities() pluginhost.Grant {
	return fileGrant(c.certPath, c.keyPath, true)
}

// Deploy writes the renewed certificate/key and rotates RabbitMQ certificates.
func (c *Connector) Deploy(_ context.Context, sb connector.Sandbox, dep connector.Deployment) error {
	return deployFiles("rabbitmq", c.certPath, c.keyPath, c.reload, sb, dep)
}
