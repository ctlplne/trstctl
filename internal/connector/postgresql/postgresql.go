...
// Connector writes PostgreSQL TLS certificate and key files.
...
// New returns a PostgreSQL connector for the configured server cert and key.
func New(certPath, keyPath string) *Connector {
	return &Connector{certPath: certPath, keyPath: keyPath, reload: []string{"pg_ctl", "reload"}}
}

// Name identifies the connector.
func (c *Connector) Name() string { return "postgresql" }
...
// Deploy writes the renewed certificate/key and reloads PostgreSQL. If reload
...
func (c *Connector) Deploy(_ context.Context, sb connector.Sandbox, dep connector.Deployment) error {
	return deployFiles("postgresql", c.certPath, c.keyPath, c.reload, sb, dep)
}
