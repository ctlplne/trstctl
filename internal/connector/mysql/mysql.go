...
// Connector writes MySQL TLS certificate and key files.
type Connector struct {
	certPath string
	keyPath  string
}

// TLSReloadAction is the fixed operator-owned action that activates MySQL's
// TLS context. Map it to mysql with ALTER INSTANCE RELOAD TLS and a private
// client option file. mysqladmin reload only refreshes grant tables.
const TLSReloadAction = "mysql-tls-reload"

var _ connector.Connector = (*Connector)(nil)

// New returns a MySQL connector for the configured server cert and key.
...
// Deploy writes the renewed certificate/key and reloads MySQL.
func (c *Connector) Deploy(_ context.Context, sb connector.Sandbox, dep connector.Deployment) error {
	return deployFiles("mysql", c.certPath, c.keyPath, sb, dep)
}
