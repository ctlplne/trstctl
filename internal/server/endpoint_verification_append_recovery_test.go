...
func TestServedEndpointObservationRecoversAfterAppendBeforeProjection(t *testing.T) {
	...
	ctx := t.Context()
	want := relay.EndpointExpectation{EndpointID: "append-won-sql-lost", Address: "127.0.0.1:5432", Fingerprint: strings.Repeat("a", 64)}
	payload, err := json.Marshal(relay.EndpointVerifyIntent{Endpoints: []relay.EndpointExpectation{want}})
	if err != nil {
		t.Fatal(err)
	}
	if err := h.store.WithTenant(ctx, h.tenant, func(tx pgx.Tx) error {
		_, err := tx.Exec(ctx, `INSERT INTO outbox (tenant_id,destination,payload,idempotency_key) VALUES ($1,$2,$3,$4)`, h.tenant, relay.KindEndpointVerify, payload, "append-recovery")
		return err
	}); err != nil {
		t.Fatal(err)
	}
	...
	// Inject failure at the real SQL projection, after the real NATS append.
	// The owned ephemeral fixture is the only database changed by this trigger.
	_, err = h.store.SystemPool().Exec(ctx, `CREATE FUNCTION qa_endpoint_projection_fail() RETURNS trigger LANGUAGE plpgsql AS $$ BEGIN IF NEW.endpoint_id='append-won-sql-lost' THEN RAISE EXCEPTION 'controlled observation projection failure'; END IF; RETURN NEW; END $$;
	CREATE TRIGGER qa_endpoint_projection_fail BEFORE INSERT OR UPDATE ON endpoint_verifications FOR EACH ROW EXECUTE FUNCTION qa_endpoint_projection_fail()`)
	if err != nil {
		t.Fatal(err)
	}
	cleanup := func() {
		...
		defer cancel()
		if _, err := h.store.SystemPool().Exec(cleanupCtx, `DROP TRIGGER IF EXISTS qa_endpoint_projection_fail ON endpoint_verifications; DROP FUNCTION IF EXISTS qa_endpoint_projection_fail()`); err != nil {
			t.Error(err)
		}
	}
	defer cleanup()
	...
	if err == nil && accepted != nil && accepted.Accepted {
		t.Error("append-won/SQL-lost report completed its claim")
	}
	...
	if _, err := h.store.GetEndpointVerification(ctx, h.tenant, want.EndpointID, string(tr.Vantage)); !store.IsNotFound(err) {
		t.Fatalf("SQL projection survived controlled failure: %v", err)
	}
	if t.Failed() {
		return
	}
