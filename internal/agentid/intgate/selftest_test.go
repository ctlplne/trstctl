...
func TestGate_IsNotVacuous(t *testing.T) {
	...
	if refs, err := NonTestCallers(root, "NewNonexistentFabricatedCtorXYZ", "internal/agentid/reach/ceiling.go", defaultSearchRoots); err != nil {
	...
	if cp, err := ControlPlaneCallers(root, "NewLocalPolicy", "internal/agentid/verify/helpers.go", defaultSearchRoots); err != nil {
	...
	if wired, err := NonTestCallers(root, "NewEngine", "internal/agentid/reach/engine/engine.go", defaultSearchRoots); err != nil {
