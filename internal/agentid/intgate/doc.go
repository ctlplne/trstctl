...
// analyzer that PROVES every internal/agentid mechanism has a real production caller and
...
//     in-scope internal/agentid packages, at least one NON-TEST caller must exist. A caller
...
//     external API (internal/agentid/api) and the licensed-outbox worker
//     (internal/agentid/orchestrator) under the FeatureAgentDelegation block; the worker
...
//
//   - DEFERRED (allow-listed): internal/agentid/verify (and its ./wasm build) is the
...
//     package (rationale in internal/agentid/verify/doc.go). Every OTHER previously
...
// exported internal/agentid constructor without either wiring it or classifying it fails the
