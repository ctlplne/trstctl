...
//
//  1. Each sanctioned seam file exists and carries no trstctl_core build constraint
//     (the core families attach in every build).
...
func TestProdCaller_SeamIsOnlySanctionedRoot(t *testing.T) {
	...
	// (1) Seam files exist and are linked in every build.
	...
	for _, s := range SanctionedSeams {
		...
		if !ok {
			t.Errorf("sanctioned seam %s carries a trstctl_core build constraint: the core families must attach in every build", s)
		}
	}
	...
	if len(notRooted) > 0 {
		...
		t.Fatalf("SEAM ASSERTION FAILED: %d REQUIRED constructor(s) are NOT reachable through a non-test caller chain rooted at the attach_families seam (a mechanism reachable only via a test helper does NOT satisfy the gate):\n  - %s",
	...
	refs, err := NonTestCallers(root, "NewLocalPolicy", "internal/agentid/verify/helpers.go", []string{"cmd"})
	if err != nil {
...
func TestSeam_DirectAttachTargets(t *testing.T) {
	...
	want := []struct{ pkg, name string }{
		{"internal/agentid/api", "NewAPIOptionsFactory"},
		{"internal/agentid/orchestrator", "NewLicensedOutboxFactory"},
		{"internal/agentid/delegation/brokerstore", "NewFailClosedBrokerPrecondition"},
		{"internal/agentid/delegation", "NewSignerGate"},
	}
	for _, w := range want {
