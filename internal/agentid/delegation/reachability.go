...
// internal/agentid/reach engine — as a PRECONDITION of the key operation, BOUND to the FINAL
...
// internal/agentid/reach, which imports only internal/crypto (so the isolated signer stays
// datastore-free and graph-free on the verify path, AN-4); the graph-walking engine that
// PRODUCES a verdict lives in the internal/agentid/reach/engine subpackage (which pulls
...
func (g *Gate) verifyReachability(tenantID string, body PreconditionsBody, now time.Time) verifyResult {
	...
	// internal/agentid/reach verifier. A stale/absent watermark, a wrong tenant/subject, a bad
