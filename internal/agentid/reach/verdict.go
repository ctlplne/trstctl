...
// engine (package internal/agentid/reach/engine), which resolves the requested authority of the
...
// internal/agentid/reach/engine, which NOTHING on the signer path imports. The signer-linked
...
// engine (internal/agentid/reach/engine) derives it from graph node labels; it lives here so the
...
// (internal/agentid/reach/engine) populates it from the tenant graph; the crypto-only digest
...
// the reachability engine (internal/agentid/reach/engine) produces its verdict's determination
