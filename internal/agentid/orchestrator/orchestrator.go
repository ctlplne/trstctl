...
// DRAINS the AGID outbox destinations enqueued by internal/agentid/api and DRIVES the AGID
// mechanisms, exactly as internal/succession/orchestrator drains pcas.succession-request and
...
// the constants internal/agentid/api enqueues under (same string value, duplicated to keep the
// API off the mechanism import graph, exactly as PCAS duplicates its destination). The
// cascade job destinations are re-exported from internal/agentid/revoke.
const (
