...
import (
	...
	vdecwasm "trstctl.com/trstctl/internal/decommission/verify/wasm"
)
...
func loadVectors(t *testing.T) []destructionRecordVector {
	...
	if err != nil {
		t.Fatalf("read %s (run UPDATE_VDEC_VECTORS=1 go test ./internal/decommission/conformance): %v", vectorPath, err)
	}
