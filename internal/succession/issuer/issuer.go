...
// (internal/rpverify.AcceptLeafTuple). The issuer chain obeys the same monotonicity and
// anchor guards as any identity (INV-2/INV-3), via internal/succession.VerifyChain.
package issuer
...
// chain (genesis + records). It reuses internal/succession.VerifyChain, so a non-monotonic,
