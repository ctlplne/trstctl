...
// internal/pqc into this package.
var algRegistry = map[crypto.Algorithm]uint64{
	...
	// internal/pqc; classified in algclass.go). The name keys MUST match the canonical
	// spellings the pqc package actually mints (internal/pqc/algorithms.go), so a real
