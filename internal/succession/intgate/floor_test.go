...
// internal/succession constructor is governed the moment it lands: it must either have a
...
func TestProdCaller_EveryConstructorHasNonTestCaller(t *testing.T) {
	...
	if len(offenders) > 0 {
		...
		t.Fatalf("PCAS-INT-CALL FLOOR FAILED: %d internal/succession constructor(s) have ONLY test-only callers and are not recorded in PendingWiring. Wire the mechanism on its owning PCAS card, or record the gap; do not add product logic in intgate:\n  - %s",
