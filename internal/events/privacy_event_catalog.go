...
var coreProductionPrivacyEventCatalog = func() []productionPrivacyEventPolicy {
	...
	catalog := []productionPrivacyEventPolicy{
		...
		entry("acme.dns01.plugin.presented",
			catalogPrivacyRule("/provider", opaque), catalogPrivacyRule("/record_name", token),
			catalogPrivacyRule("/detail", opaque)),
		entry("acme.dns01.plugin.cleaned",
			catalogPrivacyRule("/provider", opaque), catalogPrivacyRule("/record_name", token),
			catalogPrivacyRule("/detail", opaque)),
		entry("acme.dns01.plugin.denied",
			catalogPrivacyRule("/provider", opaque), catalogPrivacyRule("/record_name", token),
			catalogPrivacyRule("/detail", opaque)),
		entry("acme.dns01.plugin.failed",
			catalogPrivacyRule("/provider", opaque), catalogPrivacyRule("/record_name", token),
			catalogPrivacyRule("/detail", opaque)),
		...
		entry("agent.jobs.role_refused", catalogPrivacyRule("/agent", exact),
			catalogPrivacyRule("/roles/*", opaque), catalogPrivacyRule("/refused_kinds/*", opaque)),
		entry("agent.jobs.envelope_refused", catalogPrivacyRule("/agent", exact),
			catalogPrivacyRule("/job_id", opaque), catalogPrivacyRule("/kind", opaque)),
		entry("agent.jobs.csr_signed", catalogPrivacyRule("/agent", exact),
			catalogPrivacyRule("/job_id", opaque), catalogPrivacyRule("/names/*", token), catalogPrivacyRule("/fingerprint", opaque)),
		entry("agent.job.credential.redeemed", catalogPrivacyRule("/agent", exact),
			catalogPrivacyRule("/job_id", opaque), catalogPrivacyRule("/attempt", opaque),
			catalogPrivacyRule("/audit_ref", opaque), catalogPrivacyRule("/ref_names/*", token)),
		entry("agent.job.credential.redemption_refused", catalogPrivacyRule("/agent", exact),
			catalogPrivacyRule("/job_id", opaque), catalogPrivacyRule("/attempt", opaque), catalogPrivacyRule("/reason", opaque)),
		entry("agent.job.receipt.rejected", catalogPrivacyRule("/agent", exact),
			catalogPrivacyRule("/job_id", opaque), catalogPrivacyRule("/outcome", opaque),
			catalogPrivacyRule("/reason", opaque), catalogPrivacyRule("/agent_fingerprint", opaque)),
		// The statement and signature are one immutable proof. Subject erasure
		// may not rewrite this signed payload and pretend the signature survived.
		rejectingEntry("agent.job.executed"),
		rejectingEntry("agent.job.failed"),
		entry("spiffe.workload.svid_issued_via_agent", catalogPrivacyRule("/agent", exact),
			catalogPrivacyRule("/node", token), catalogPrivacyRule("/selectors/*", token),
			catalogPrivacyRule("/x509", opaque), catalogPrivacyRule("/jwt", opaque)),
		...
		entry("certificate.expiring",
			catalogPrivacyRule("/certificate_id", opaque), catalogPrivacyRule("/serial", opaque),
			catalogPrivacyRule("/not_after", opaque)),
		...
		entry("connector.rollback.requested",
			catalogPrivacyRule("/connector", opaque), catalogPrivacyRule("/target", token),
			catalogPrivacyRule("/target_id", opaque), catalogPrivacyRule("/identity_id", opaque),
			catalogPrivacyRule("/target_config", jsonID),
			catalogPrivacyRule("/predecessor_fingerprint", opaque), catalogPrivacyRule("/predecessor_serial", opaque),
			catalogPrivacyRule("/successor_fingerprint", opaque), catalogPrivacyRule("/reason", clear),
			catalogPrivacyRule("/requested_by", exact), catalogPrivacyRule("/required_agent_id", opaque),
			catalogPrivacyRule("/required_agent_role", opaque)),
		...
		entry("ephemeral.issued",
			...
			catalogPrivacyRule("/recovered", opaque)),
		...
		// Executable modules deliberately have no rewrite rule. If a module
		// contains the erased subject, erasure fails closed instead of changing
		// authorization logic or retaining an unreported subject literal.
		entry("policy.version.authored",
			catalogPrivacyRule("/id", opaque), catalogPrivacyRule("/kind", opaque),
			catalogPrivacyRule("/module_sha256", opaque), catalogPrivacyRule("/package", opaque),
			catalogPrivacyRule("/query", opaque), catalogPrivacyRule("/description", clear),
			catalogPrivacyRule("/change_ref", clear), catalogPrivacyRule("/evidence_refs/*", clear),
			catalogPrivacyRule("/author", exact)),
		entry("policy.version.activated",
			catalogPrivacyRule("/id", opaque), catalogPrivacyRule("/kind", opaque),
			catalogPrivacyRule("/reason", clear), catalogPrivacyRule("/evidence_refs/*", clear),
			catalogPrivacyRule("/activated_by", exact), catalogPrivacyRule("/previous_id", opaque),
			catalogPrivacyRule("/previous_module_sha256", opaque), catalogPrivacyRule("/previous_package", opaque),
			catalogPrivacyRule("/previous_query", opaque)),
		entry("policy.version.rolled_back",
			catalogPrivacyRule("/id", opaque), catalogPrivacyRule("/kind", opaque),
			catalogPrivacyRule("/reason", clear), catalogPrivacyRule("/evidence_refs/*", clear),
			catalogPrivacyRule("/rolled_back_by", exact), catalogPrivacyRule("/rollback_to_id", opaque),
			catalogPrivacyRule("/rollback_to_module_sha256", opaque), catalogPrivacyRule("/rollback_to_package", opaque),
			catalogPrivacyRule("/rollback_to_query", opaque)),
		...
		entry("secret.version.written",
			...
			catalogPrivacyRule("/sealed", opaque),
			catalogPrivacyRule("/envelope/format", opaque),
			catalogPrivacyRule("/envelope/version", opaque),
			catalogPrivacyRule("/envelope/wrapped_dek", opaque),
			catalogPrivacyRule("/envelope/dek_nonce", opaque),
			catalogPrivacyRule("/envelope/nonce", opaque),
			catalogPrivacyRule("/envelope/ciphertext", opaque),
		...
		entry("ssh.cert.issued",
			...
			catalogPrivacyRule("/profile", opaque)),
		entry("ssh.cert.revoked",
			catalogPrivacyRule("/serial", opaque), catalogPrivacyRule("/key_id", token),
			catalogPrivacyRule("/reason", clear)),
		entry("ssh.host.retired",
			catalogPrivacyRule("/id", opaque), catalogPrivacyRule("/tenant_id", opaque),
			catalogPrivacyRule("/host", token), catalogPrivacyRule("/source_id", opaque),
			catalogPrivacyRule("/run_id", opaque), catalogPrivacyRule("/identity_id", opaque),
			catalogPrivacyRule("/reason", clear), catalogPrivacyRule("/status", opaque),
			catalogPrivacyRule("/recorded_at", opaque)),
		entry("ssh.trust_rollout.recorded",
			catalogPrivacyRule("/id", opaque), catalogPrivacyRule("/tenant_id", opaque),
			catalogPrivacyRule("/source_id", opaque), catalogPrivacyRule("/target_hosts/*", token),
			catalogPrivacyRule("/candidate_ca_fingerprint", opaque),
			catalogPrivacyRule("/reload_command", clear), catalogPrivacyRule("/health_command", clear),
			catalogPrivacyRule("/rollback_plan", clear), catalogPrivacyRule("/status", opaque),
			catalogPrivacyRule("/confirmed", opaque), catalogPrivacyRule("/recorded_at", opaque)),
