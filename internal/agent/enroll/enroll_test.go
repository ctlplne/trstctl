...
func newCSRWithDNS(t *testing.T, cn string, dnsNames []string) []byte {
	t.Helper()
...
// common name or requested identity SANs differ, and accepts the matching one —
// scoping a leaked token.
func TestAllowedIdentityPin(t *testing.T) {
	...
	// A matching CN with a rogue SAN is also refused.
	tok2, _ := a.IssueBootstrapToken(ctx, tenantA, "payments-agent")
	if _, err := a.EnrollBootstrap(ctx, tok2, newCSRWithDNS(t, "payments-agent", []string{"attacker-agent"})); !errors.Is(err, enroll.ErrBadToken) {
		t.Fatalf("mismatched SAN = %v, want ErrBadToken", err)
	}

	// Matching identity is accepted.
	tok3, _ := a.IssueBootstrapToken(ctx, tenantA, "payments-agent")
	if _, err := a.EnrollBootstrap(ctx, tok3, newCSR(t, "payments-agent")); err != nil {
