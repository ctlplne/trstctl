...
type ReportJobResultRequest struct {
	...
	// the detached signature; omission or alteration makes the terminal report
...
func SignedReport(id StatementSigner, tenantID, commonName string, jobID int64, attempt int,
	...
	return SignedReportWithCustody(id, tenantID, commonName, jobID, attempt, outcome, detail,
		evidenceDigest, "", custody.Record{}, issuedAtUnix)
}

// SignedReportWithCustody builds the v2 terminal statement for an installed
// certificate whose private key was generated by the reporting agent.
func SignedReportWithCustody(id StatementSigner, tenantID, commonName string, jobID int64, attempt int,
	outcome, detail, evidenceDigest, credentialFingerprint string, record custody.Record,
	issuedAtUnix int64) (*ReportJobResultRequest, error) {
	statement := JobReceiptStatement{
		...
		DetailDigest: DetailDigest(detail), CredentialFingerprint: credentialFingerprint,
		Custody: record, IssuedAtUnix: issuedAtUnix,
	}
	...
	var wireCustody *custody.Record
	if statement.hasCustody() {
		copy := record
		wireCustody = &copy
	}
	return &ReportJobResultRequest{
		JobID: jobID, Outcome: outcome, Detail: detail, EvidenceDigest: evidenceDigest,
		CredentialFingerprint: credentialFingerprint, Custody: wireCustody,
