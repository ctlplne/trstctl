...
func TestSignerMetricsExposeUpAndRestarts(t *testing.T) {
	...
	m.Observe(false, 0)
	if got := renderProm(t, reg); !strings.Contains(got, "trustctl_signer_up 0") {
	...
	if !strings.Contains(got, "trustctl_signer_up 1") {
		t.Errorf("expected signer_up 1 while healthy:\n%s", got)
	}
	if !strings.Contains(got, "trustctl_signer_restarts_total 2") {
...
func TestSignerRestartsCounterIsMonotonic(t *testing.T) {
	...
	// exported counter.
	m.Observe(true, 1)
	if got := renderProm(t, reg); !strings.Contains(got, "trustctl_signer_restarts_total 3") {
		t.Errorf("counter must not go backwards; want 3:\n%s", got)
	}
	// A higher value adds only the delta.
	m.Observe(true, 5)
	if got := renderProm(t, reg); !strings.Contains(got, "trustctl_signer_restarts_total 5") {
