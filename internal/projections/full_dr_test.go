...
func seedRecoveredFromPostgresTables(t *testing.T, st *store.Store) {
	...
	err := st.WithTenant(ctx, tenantA, func(tx pgx.Tx) error {
		statements := []struct {
			...
			{`INSERT INTO ca_crls (tenant_id, ca_id, crl_number, crl_der, this_update, next_update) VALUES ($1, $2, $3, $4, $5, $6)`, []any{tenantA, caID, int64(1), []byte{0x30, 0x03, 0x01}, now, now.Add(24 * time.Hour)}},
			{`INSERT INTO ca_issued_certs (tenant_id, ca_id, serial, issued_at, reason_code) VALUES ($1, $2, $3, $4, $5)`, []any{tenantA, caID, "issued-01", now, 0}},
			{`INSERT INTO certificate_profiles (id, tenant_id, name, version, spec, active, created_by) VALUES ($1, $2, $3, $4, $5::jsonb, $6, $7)`, []any{"00000000-0000-0000-0000-00000000a004", tenantA, "server", 1, `{"max_ttl":"24h"}`, true, "dr-test"}},
