...
func componentSchemas() map[string]*Schema {
	...
	policyVersion := object(map[string]*Schema{
		...
		"rollback_available": {Type: "boolean", Description: "This active version has a recorded predecessor that the rollback operation can restore. Authorization and runtime enforcement are checked separately. Older idempotent responses may omit this field; treat absence as unknown and refresh the version list."},
	...
	policyVersionList := object(map[string]*Schema{
		...
		"enforcement": object(map[string]*Schema{
			"enabled":       {Type: "boolean"},
			"module_sha256": str(),
		}, "enabled"),
	}, "items", "counts", "enforcement")
