...
func openAPIOperationIDs(t *testing.T, doc map[string]any) map[string]bool {
	...
	// F38 adds the caller-attenuated, effect-free ephemeral API-key plan.
	...
	// code-signing previews, bringing the served contract to 428 operations.
	if len(out) != 428 {
		t.Fatalf("OpenAPI operationIds = %d, want 428", len(out))
	}
	return out
}
