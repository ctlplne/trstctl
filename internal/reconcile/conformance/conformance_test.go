...
import (
	...
	xrecverify "trstctl.com/trstctl/internal/reconcile/verify"
...
// TestEdition_CoreBuildLinksXRECAndNoEE: XREC is part of the core since
// 2026-09-20, so the trstctl_core build must link it and still link zero ee/.
func TestEdition_CoreBuildLinksXRECAndNoEE(t *testing.T) {
	root := repoRoot(t)
	...
	for _, line := range strings.Split(string(out), "\n") {
		...
		if strings.HasPrefix(line, "trstctl.com/trstctl/ee/") {
			t.Fatalf("core build links an ee/ package %s", line)
		}
		if line == "trstctl.com/trstctl/internal/reconcile" {
			linked = true
		}
	}
	if !linked {
		t.Fatal("core-only build does not link internal/reconcile; XREC must attach in every build")
	}
}

func TestEdition_AllXRECPackagesAreCore(t *testing.T) {
	root := repoRoot(t)
	repo := repoRootHandle(t)
	// Spelled by concatenation so this file does not match its own guard.
	foreignSPDX := [][]byte{[]byte("SPDX-License-Identifier: " + "MPL-2.0"), []byte("SPDX-License-Identifier: " + "LicenseRef-trstctl-EE")}
	err := filepath.WalkDir(filepath.Join(root, "internal", "reconcile"), func(path string, d os.DirEntry, err error) error {
		...
		if !bytes.Contains(raw, []byte("SPDX-License-Identifier: BUSL-1.1")) {
			return fmt.Errorf("%s missing BUSL-1.1 SPDX", rel)
		}
		for _, foreign := range foreignSPDX {
			if bytes.Contains(raw, foreign) {
				return fmt.Errorf("%s carries a foreign SPDX header inside XREC: %s", rel, foreign)
			}
		}
		return nil
	})
...
func vectorsDir(t testContext) string {
	t.Helper()
	return filepath.Join(repoRoot(t), "internal", "reconcile", "conformance", "testdata", "vectors")
}
