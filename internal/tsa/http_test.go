...
func TestTimeStampHTTPPostOpenSSLTSVerify(t *testing.T) {
	...
	out, err = exec.Command(ossl, "ts", "-verify", "-queryfile", reqPath, "-in", respPath, "-CAfile", rootPath, "-untrusted", tsaPath).CombinedOutput() // #nosec G204 -- test executes a fixed local tool or fixture it built itself (CWE-78)
	if err != nil {
