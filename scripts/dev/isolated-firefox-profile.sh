...
# Usage: scripts/dev/isolated-firefox-profile.sh <control-plane.crt> [profile-dir]
set -eu
CERT=${1:?usage: isolated-firefox-profile.sh <control-plane.crt> [profile-dir]}
...
echo "profile ready: firefox --profile \"$(cd "$PROFILE" && pwd)\" https://127.0.0.1:9443"
