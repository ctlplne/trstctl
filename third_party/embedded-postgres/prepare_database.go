...
func defaultCreateDatabase(port uint32, username, password, database string) (err error) {
	...
	if _, err := db.Exec("CREATE DATABASE " + pgx.Identifier{database}.Sanitize()); err != nil {
		return errorCustomDatabase(database, err)
	}
...
func openDatabaseConnection(port uint32, username string, password string, database string) (driver.Connector, error) {
	config, err := pgx.ParseConfig(fmt.Sprintf("host=localhost port=%d user=%s password=%s dbname=%s sslmode=disable",
		port,
		username,
		password,
		database))
