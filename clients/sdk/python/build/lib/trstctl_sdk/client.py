...
class TrstctlClient:
    ...
    def __init__(
        ...
        *,
        ...
        if opener is None and ca_file:
            # A self-hosted control plane usually presents a certificate from a
            # private CA. Validate against exactly that bundle; never disable
            # verification to make a private CA work.
    ...
    def from_env(cls, **kwargs: Any) -> "TrstctlClient":
        """Build a client from TRSTCTL_SERVER, TRSTCTL_TOKEN, TRSTCTL_TENANT, and TRSTCTL_CA_FILE (the CA bundle that signed the control plane's certificate; the same variable the CLI reads)."""

        base_url = os.environ.get("TRSTCTL_SERVER") or os.environ.get("TRSTCTL_ENDPOINT")
        if not base_url:
            raise ValueError("TRSTCTL_SERVER is required")
        kwargs.setdefault("ca_file", os.environ.get("TRSTCTL_CA_FILE") or None)
        return cls(
            base_url=base_url,
