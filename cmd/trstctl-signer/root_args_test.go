...
func TestSignerCLIArgumentBoundary(t *testing.T) {
	if os.Getenv("TRSTCTL_TEST_Signer_ARGS_HELPER") == "1" {
	...
	for _, tc := range []struct {
		...
		{"version", false, func(ctx context.Context) *exec.Cmd {
			return exec.CommandContext(ctx, "./daemon-test.exe", "-test.run=^TestSignerCLIArgumentBoundary$", "--", "--version")
		}},
		{"help", false, func(ctx context.Context) *exec.Cmd {
			return exec.CommandContext(ctx, "./daemon-test.exe", "-test.run=^TestSignerCLIArgumentBoundary$", "--", "--help")
		}},
		{"unexpected", true, func(ctx context.Context) *exec.Cmd {
			return exec.CommandContext(ctx, "./daemon-test.exe", "-test.run=^TestSignerCLIArgumentBoundary$", "--", "--version", "qa-private-operand")
		}},
		{"after-separator", true, func(ctx context.Context) *exec.Cmd {
			return exec.CommandContext(ctx, "./daemon-test.exe", "-test.run=^TestSignerCLIArgumentBoundary$", "--", "--version", "--", "qa-private-operand")
		}},
		{"ignored-tail-flags", true, func(ctx context.Context) *exec.Cmd {
			return exec.CommandContext(ctx, "./daemon-test.exe", "-test.run=^TestSignerCLIArgumentBoundary$", "--", "--version", "qa-private-operand", "--version")
		}},
	} {
		t.Run(tc.name, func(t *testing.T) {
			...
			defer cancel()
			cmd := tc.command(ctx)
			cmd.Env = append(os.Environ(), "TRSTCTL_TEST_Signer_ARGS_HELPER=1")
			cmd.Dir = workingDir
