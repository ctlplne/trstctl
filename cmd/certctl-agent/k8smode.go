...
func runKubernetes(ctx context.Context, o agentOptions, k k8sOptions) error {
	...
	if k.secret != "" {
		...
		fmt.Printf("certctl-agent: published identity into secret %s\n", k.secret)
	}
	...
	case k.signerURL == "":
		fmt.Fprintln(os.Stderr, "certctl-agent: --cert-manager-issuer set but --bridge-signer-url is empty; cert-manager bridge disabled")
	default:
		bridge = k8s.NewBridge(client, k8s.NewHTTPSigner(k.signerURL, nil), k.issuer, k.group)
		fmt.Printf("certctl-agent: cert-manager bridge active for issuer %q\n", k.issuer)
	}
	...
	for {
		...
		case <-ticker.C:
			...
			if err != nil {
				fmt.Fprintln(os.Stderr, "certctl-agent: cert-manager reconcile:", err)
			} else if n > 0 {
				fmt.Printf("certctl-agent: signed %d cert-manager request(s)\n", n)
			}
		}
	}
}
