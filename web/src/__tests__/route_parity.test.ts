...
describe("route-level served-feature parity", () => {
  it("requires every feature-map row to declare explicit served maturity metadata", () => {
    ...
    expect(featureCoverageItems.filter((item) => item.servedState === "served")).toHaveLength(15);
    ...
    expect(featureCoverageItems.filter((item) => item.servedState === "library")).toHaveLength(30);
