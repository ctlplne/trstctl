...
/** S-C9: the primitives pin the refinement rules — one eyebrow style, one
 * inline-number style — so a class-cluster drift shows up here, not in a
 * design review. */

describe("typography primitives (S-C9)", () => {
  it("Eyebrow renders the single tracked-uppercase micro-label cluster", () => {
    render(<Eyebrow>Needs action</Eyebrow>);
    const eyebrow = screen.getByText("Needs action");
    expect(eyebrow.tagName).toBe("SPAN");
    for (const cls of ["text-caption", "font-semibold", "uppercase", "tracking-wide", "text-muted-foreground"]) {
