...
1306 annotated sites across 26 rules. Each row is
...
| `internal/config/config.go:2181` | identifier/constant matching the secret-name heuristic; no credential value present (CWE-798) |
| `internal/config/config.go:2845` | identifier/constant matching the secret-name heuristic; no credential value present (CWE-798) |
...
| `internal/server/agentchannel_served_test.go:770` | fixture sequences are single-digit seconds (CWE-190). |
...
| `deploy/docker/dist_test.go:482` | test executes a fixed local tool or fixture it built itself (CWE-78) |
| `deploy/docker/dist_test.go:585` | test executes a fixed local tool or fixture it built itself (CWE-78) |
| `deploy/docker/dist_test.go:881` | test executes a fixed local tool or fixture it built itself (CWE-78) |
| `deploy/docker/dist_test.go:946` | test executes a fixed local tool or fixture it built itself (CWE-78) |
...
| `deploy/docker/dist_test.go:825` | npm fixture tree in t.TempDir; mirrors a real package layout, nothing secret (CWE-276) |
...
| `internal/server/agentchannel_served_test.go:1459` | fixture tree in a test tempdir; the mode is part of the fixture (CWE-276) |
...
| `internal/server/server.go:2133` | served CA certificate directory; the PEM is public material (CWE-276) |
...
### G304 — CWE-22 Path traversal (file inclusion via variable) (350 sites)
...
| `deploy/docker/dist_test.go:897` | test reads its own fixture/tempdir path (CWE-22) |
...
| `internal/config/config.go:2254` | the config loader reading the operator's own config file (CWE-22) |
...
| `internal/server/server.go:2060` | operator-configured local file path from deployment config (CWE-22) |
| `internal/server/server.go:2137` | same operator-configured directory as the target certificate (CWE-22) |
...
| `deploy/docker/dist_test.go:828` | non-secret npm fixture manifest in t.TempDir (CWE-276) |
| `deploy/docker/dist_test.go:831` | fixture file in a test tempdir; the mode is part of the fixture (CWE-276) |
| `deploy/docker/dist_test.go:837` | fake npm shim in a test tempdir must be executable (CWE-276) |
...
| `internal/server/server.go:2143` | served CA certificate PEM is public material (CWE-276) |
...
| `internal/crypto/mtls/server.go:445` | localhost liveness probe of this process's own ephemeral self-signed listener; no credential, no data (CWE-295) |
...
| `internal/cli/cli.go:483` | idempotency-key uniqueness suffix; deliberately outside the AN-3 boundary, not a secret (CWE-338) |
