...
1029 annotated sites across 25 rules. Each row is
generated from the `#nosec` comment at that exact line; edit the source,
not this file.

### G101 — CWE-798 Use of hardcoded credentials (224 sites)
...
| `internal/config/config.go:1814` | identifier/constant matching the secret-name heuristic; no credential value present (CWE-798) |
| `internal/config/config.go:2395` | identifier/constant matching the secret-name heuristic; no credential value present (CWE-798) |
...
| `internal/store/store_isolation_test.go:216` | test reads its own fixture/tempdir path (CWE-22, CWE-367) |
| `internal/store/store_isolation_test.go:262` | test reads its own fixture/tempdir path (CWE-22, CWE-367) |
...
| `internal/server/server.go:1781` | served CA certificate directory; the PEM is public material (CWE-276) |
...
| Location | Reason |
|---|---|
| `cmd/trstctl-agent/main.go:288` | 0700 on a directory: the execute bit is required to traverse it (CWE-276) |
...
| `internal/config/config.go:1887` | the config loader reading the operator's own config file (CWE-22) |
...
| `internal/server/server.go:1743` | operator-configured local file path from deployment config (CWE-22) |
...
| `internal/server/server.go:1785` | served CA certificate PEM is public material (CWE-276) |
...
| `cmd/trstctl-agent/main.go:345` | reconnect jitter, not a security decision (CWE-338) |
