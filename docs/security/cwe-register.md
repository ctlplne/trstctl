...
| `internal/agent/transport/agentservice.go:750` | an RPC method name, not a credential. The material this |
| `internal/agent/transport/agentservice.go:757` | an RPC method name. This call carries a CSR up and returns |
...
| `internal/config/config.go:2260` | identifier/constant matching the secret-name heuristic; no credential value present (CWE-798) |
| `internal/config/config.go:2958` | identifier/constant matching the secret-name heuristic; no credential value present (CWE-798) |
...
| `internal/server/itsm_served_test.go:25` | fabricated fixture credential/identifier; the test needs the shape, no value is real (CWE-798) |
| `internal/server/itsm_served_test.go:34` | fabricated fixture credential/identifier; the test needs the shape, no value is real (CWE-798) |
| `internal/server/itsm_served_test.go:126` | fabricated fixture credential/identifier; the test needs the shape, no value is real (CWE-798) |
| `internal/server/itsm_served_test.go:135` | fabricated fixture credential/identifier; the test needs the shape, no value is real (CWE-798) |
| `internal/server/itsm_served_test.go:148` | fabricated fixture credential/identifier; the test needs the shape, no value is real (CWE-798) |
| `internal/server/itsm_served_test.go:184` | fabricated fixture credential/identifier; the test needs the shape, no value is real (CWE-798) |
...
| `internal/server/protocols_served_test.go:321` | fabricated fixture credential/identifier; the test needs the shape, no value is real (CWE-798) |
| `internal/server/protocols_served_test.go:385` | opaque fake secret reference used only by the local test provider (CWE-798). |
| `internal/server/protocols_served_test.go:621` | fabricated fixture credential/identifier; the test needs the shape, no value is real (CWE-798) |
| `internal/server/protocols_served_test.go:730` | fabricated secret reference, never raw credential material (CWE-798) |
| `internal/server/protocols_served_test.go:874` | fabricated fixture credential/identifier; the test needs the shape, no value is real (CWE-798) |
| `internal/server/protocols_served_test.go:911` | fabricated fixture credential/identifier; the test needs the shape, no value is real (CWE-798) |
| `internal/server/protocols_served_test.go:982` | fabricated fixture credential/identifier; the test needs the shape, no value is real (CWE-798) |
| `internal/server/protocols_served_test.go:1059` | fabricated fixture credential/identifier; the test needs the shape, no value is real (CWE-798) |
| `internal/server/protocols_served_test.go:1259` | fabricated fixture credential/identifier; the test needs the shape, no value is real (CWE-798) |
| `internal/server/response_integrations_served_test.go:31` | fabricated fixture credential/identifier; the test needs the shape, no value is real (CWE-798) |
| `internal/server/response_integrations_served_test.go:51` | fabricated fixture credential/identifier; the test needs the shape, no value is real (CWE-798) |
...
| `ee/enterpriseauth/auth_ldap_served_test.go:129` | test executes a fixed local tool or fixture it built itself (CWE-78) |
| `ee/enterpriseauth/auth_ldap_served_test.go:134` | test executes a fixed local tool or fixture it built itself (CWE-78) |
| `ee/enterpriseauth/auth_ldap_served_test.go:140` | test executes a fixed local tool or fixture it built itself (CWE-78) |
| `ee/enterpriseauth/auth_ldap_served_test.go:157` | test executes a fixed local tool or fixture it built itself (CWE-78) |
| `ee/enterpriseauth/auth_ldap_served_test.go:168` | test executes a fixed local tool or fixture it built itself (CWE-78) |
| `ee/enterpriseauth/auth_ldap_served_test.go:174` | test executes a fixed local tool or fixture it built itself (CWE-78) |
...
| `internal/server/server.go:2378` | served CA certificate directory; the PEM is public material (CWE-276) |
...
| `internal/agent/relay/tomcat_activation_test.go:49` | fixed filenames below t.TempDir, no external input (CWE-22). |
...
| `internal/config/config.go:2333` | the config loader reading the operator's own config file (CWE-22) |
...
| `internal/license/license.go:352` | operator-supplied license file path (CWE-22) |
| `internal/notify/response_buffer_guard_test.go:37` | test reads its own fixture/tempdir path (CWE-22, CWE-367) |
...
| `internal/pqc/prepared_leaf_openssl_test.go:58` | fixed server.log filename in this test's private TempDir; no user path. |
...
| `internal/server/server.go:2305` | operator-configured local file path from deployment config (CWE-22) |
| `internal/server/server.go:2382` | same operator-configured directory as the target certificate (CWE-22) |
...
| `internal/server/server.go:2388` | served CA certificate PEM is public material (CWE-276) |
...
| Location | Reason |
|---|---|
| `internal/crypto/certinfo/certinfo.go:421` | display/lookup fingerprint in the industry-standard form; not a security control (CWE-328) |
...
| `internal/server/issuance_dispatcher_test.go:1440` | fixed-shape test data; the index is in range by construction (CWE-118) |
...
| `internal/server/compose_e2e_tls_test.go:229` | fixed script filename below this test's private t.TempDir; test call text is file content, never a path (CWE-22). |
| `internal/server/compose_e2e_tls_test.go:357` | fixed invalid.pem filename below this test's private t.TempDir; deliberately invalid public fixture bytes are content only (CWE-22). |
