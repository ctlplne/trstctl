...
993 annotated sites across 24 rules. Each row is
...
| `internal/api/openapi.go:157` | identifier/constant matching the secret-name heuristic; no credential value present (CWE-798) |
...
| `internal/config/config.go:1845` | identifier/constant matching the secret-name heuristic; no credential value present (CWE-798) |
...
| `internal/notify/notify.go:50` | identifier/constant matching the secret-name heuristic; no credential value present (CWE-798) |
...
| `tools/trstctllint/keymaterial/keymaterial_test.go:44` | fabricated fixture credential/identifier; the test needs the shape, no value is real (CWE-798) |
| `tools/trstctllint/keymaterial/keymaterial_test.go:80` | fabricated fixture credential/identifier; the test needs the shape, no value is real (CWE-798) |
...
### G115 — CWE-190 Integer overflow or wraparound (123 sites)
...
| `internal/server/protocol_mounts.go:758` | DER lengths of certificates/keys are orders of magnitude under the uint32 bound (CWE-190) |
| `internal/server/protocol_mounts.go:760` | DER lengths of certificates/keys are orders of magnitude under the uint32 bound (CWE-190) |
...
| `internal/store/snapshot.go:150` | event sequence/count fits int64 by construction; the column is a Postgres bigint (CWE-190) |
...
| `docs/docs_test.go:2464` | test walks the repo's own checkout; no hostile symlink exposure (CWE-367) |
...
| `docs/protect_guards_test.go:843` | test reads its own fixture/tempdir path (CWE-22, CWE-367) |
| `docs/protect_guards_test.go:906` | test reads its own fixture/tempdir path (CWE-22, CWE-367) |
| `docs/protect_guards_test.go:4410` | test reads its own fixture/tempdir path (CWE-22, CWE-367) |
...
| `internal/store/store_isolation_test.go:195` | test reads its own fixture/tempdir path (CWE-22, CWE-367) |
| `internal/store/store_isolation_test.go:241` | test reads its own fixture/tempdir path (CWE-22, CWE-367) |
...
| `internal/connector/netscaler/netscaler.go:197` | cookie on an outbound API request; response-cookie attributes do not apply (CWE-1004) |
...
| `internal/server/agentchannel_served_test.go:1200` | fixture tree in a test tempdir; the mode is part of the fixture (CWE-276) |
...
| `internal/server/server.go:1738` | served CA certificate directory; the PEM is public material (CWE-276) |
...
### G304 — CWE-22 Path traversal (file inclusion via variable) (259 sites)
...
| `docs/ai_surface_placement_test.go:27` | test reads its own fixture/tempdir path (CWE-22, CWE-367) |
| `docs/docs_drift_test.go:376` | test reads its own fixture/tempdir path (CWE-22) |
| `docs/est_differential_test.go:190` | test reads its own fixture/tempdir path (CWE-22, CWE-367) |
...
| `internal/server/protect_correct102_guard_test.go:100` | test reads its own fixture/tempdir path (CWE-22, CWE-367) |
| `internal/server/protocol_mounts.go:538` | operator-configured local file path from deployment config (CWE-22) |
| `internal/server/protocol_mounts.go:645` | operator-configured local file path from deployment config (CWE-22) |
| `internal/server/protocol_mounts.go:710` | operator-configured local file path from deployment config (CWE-22) |
...
| `internal/server/run.go:834` | operator-configured local file path from deployment config (CWE-22) |
| `internal/server/run.go:1187` | operator-configured local file path from deployment config (CWE-22) |
| `internal/server/run_connectors_test.go:89` | test reads its own fixture/tempdir path (CWE-22) |
| `internal/server/server.go:1700` | operator-configured local file path from deployment config (CWE-22) |
...
| `internal/server/server.go:1742` | served CA certificate PEM is public material (CWE-276) |
