...
| `ownership.assigned.actor-reason` | Immutable event fields `assigned_by/reason`; current `ownership_assignments` rows contain only opaque owner, inventory, event, sequence, and time references. | Event privacy policy pseudonymizes an exact matching actor and clears free-text reason material while preserving opaque decision structure. The current projection contains no copied actor or reason. |
