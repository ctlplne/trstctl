...
| `discovery_sources.config` | `discovery_sources.config` | Subject export includes matching JSON values; subject erasure pseudonymizes exact matching JSON string values; retention clears stale source-config PII paths after discovery evidence ages out. |
| `discovery_findings.metadata` | `discovery_findings.metadata` | Subject export includes matching JSON values; subject erasure pseudonymizes exact matching JSON string values; retention clears stale finding-metadata PII paths after discovery evidence ages out. |
