...
| **AN-9 — editions boundary** | Commercial code lives only under `ee/`; core never imports it, and a core-only build links zero `ee/` packages. Multi-tenancy, the crypto boundary, audit and export rights, and the offline license verifier stay in the MPL core. | New commercial capability lands under `ee/` behind the editions build fence; core packages gain no `ee/` import; the core-only build still links and its tests still pass. |
