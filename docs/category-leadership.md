...
| COMPETE-040 | CAP-KEY-05 Multiple algorithms (RSA / ECDSA / Ed25519) + Enterprise/PQC | Served proof recorded | `docs/features/issuance-and-cas.md` and `docs/features/lifecycle-and-pqc.md` document the buyer-visible profile path for classical RSA, ECDSA, and Ed25519 in the MPL core, plus proprietary Enterprise/PQC hybrid transition, ML-DSA, and SLH-DSA choices under `ee/pqc` and `ee/pqcmigration`. `internal/server/crypto_agility_served_test.go` `TestServedCryptoAgilityProfilesValidateBoundaryAlgorithms` proves `POST /api/v1/profiles` rejects unsupported labels and round-trips the MPL-core classical algorithms; PACKAGING-007 moves the PQC capability, APIs, UI, and tests behind `ee/` rather than counting it as a Community/core MEET. |
...
| COMPETE-044 | CAP-POL-06 Tamper-evident / signed audit log | Served proof recorded | `docs/features/policy-and-governance.md` documents the buyer receipt for `GET /api/v1/audit/events` and `GET /api/v1/audit/export`: tenant-scoped audit replay, hash-chained records, offline `VerifyChain` detection, and a JOSE-signed evidence bundle. `internal/audit/chain_test.go` `TestChainDetectsTampering` proves record alteration and dropped events break the chain. `internal/audit/audit_test.go` `TestEvidenceBundleVerifies` proves exported bundles verify and tampered JWS payloads fail. `internal/server/breakglass_served_test.go` `TestServedBreakglassReconcileRecordsAuditChain` proves a real served mutation is reconciled into the tenant audit log and returned through the served audit route with a verifiable hash chain. |

These eleven rows close the automatically fixable REPORT-004 source gaps. They are
