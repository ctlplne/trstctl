...
func TestStorageTenancyRegressionGuardsStayRequired(t *testing.T) {
	...
	for _, want := range []string{
		...
		"scripts/ci/coverage-normalize.sh",
		"$(GO_PACKAGES)",
		"./tools/trstctllint",
		"-vettool",
	} {
...
func TestSchemaCompatibilityStrengthGuardsStayRequired(t *testing.T) {
	...
	for _, file := range []struct {
		...
		{"../internal/store/snapshot.go", []string{"SnapshotFormatVersion = 11", "WHERE format_version = $1", "SELECT tenant_id, payload FROM read_model_snapshots"}},
	} {
...
func TestSecurityPostureStrengthGuardsStayRequired(t *testing.T) {
	for _, tc := range []struct {
		...
		{"../internal/api", "TestGuardAppliesPerTenantRateLimiter", "SEC-003"},
		{"../internal/authz", "TestRegistrationAuthoritySeparation", "SEC-003"},
		{"../internal/server", "TestServedMutationIdempotencyReplayMatrix", "SEC-003"},
...
func TestSpineStrengthGuardsStayRequired(t *testing.T) {
	...
	for _, want := range []string{
		"const SnapshotFormatVersion = 11",
...
func TestReleaseGuardrailCommandsStayFirstClass(t *testing.T) {
	...
	for _, want := range []string{
		...
		"run: make test",
		"run: make lint",
		"bash scripts/ci/coverage-normalize_selftest.sh",
		"run: npm ci",
		"run: npm run lint",
